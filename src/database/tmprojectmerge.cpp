                                      ...
                                      wxArrayLong *newids)
{
    ...
    if (m_DB->DataBaseQueryNoResults(
            wxString::Format(myQuery, m_MasterFileName.GetFullName(), myTempTableName, m_SlaveFileName.GetFullName(),
...
bool tmProjectMerge::_MergeGeom(const wxString &geomtablename, const wxString &aatablename, int geomtype)
{
    ...
    if (m_DB->DataBaseQuery(wxString::Format(_T("SELECT OBJECT_ID FROM %s.%s ORDER BY OBJECT_ID DESC LIMIT 1"),
                                             m_MasterFileName.GetFullName(), geomtablename), true) == false) {
        return false;
    }
    ...
    if (m_DB->DataBaseQuery(
            wxString::Format(myQuery, m_MasterFileName.GetFullName(), geomtablename, m_SlaveFileName.GetFullName(),
                             geomtablename), true) == false) {
        return false;
    }
...
bool tmProjectMerge::CheckSimilar()
{
    ...
    wxString myQuery = wxString::Format(
            _T("SELECT c.LAYER_INDEX, c.LAYER_NAME, d.LAYER_NAME, c.TYPE_CD, d.TYPE_CD FROM %s.thematic_layers c, %s.thematic_layers d WHERE c.LAYER_INDEX = d.LAYER_INDEX AND (c.LAYER_NAME <> d.LAYER_NAME OR c.TYPE_CD <> d.TYPE_CD)"),
            m_MasterFileName.GetFullName(), m_SlaveFileName.GetFullName());
    ...
    myQuery = wxString::Format(
            _T("SELECT c.CATALOG_ID, c.CODE, c.DESCRIPTION_0, d.CATALOG_ID, d.CODE, d.DESCRIPTION_0 FROM %s.dmn_catalog c, %s.dmn_catalog d WHERE c.CATALOG_ID = d.CATALOG_ID AND (c.CODE <> d.CODE OR c.DESCRIPTION_0 <> d.DESCRIPTION_0)"),
            m_MasterFileName.GetFullName(), m_SlaveFileName.GetFullName());
    ...
    myQuery = wxString::Format(
            _T("SELECT c.ATTRIBUT_ID, c.LAYER_INDEX, c.ATTRIBUT_NAME, d.ATTRIBUT_ID, d.LAYER_INDEX, d.ATTRIBUT_NAME FROM %s.dmn_layer_attribut c, %s.dmn_layer_attribut d WHERE c.ATTRIBUT_ID = d.ATTRIBUT_ID AND (c.LAYER_INDEX <> d.LAYER_INDEX or c.ATTRIBUT_NAME <> d.ATTRIBUT_NAME)"),
            m_MasterFileName.GetFullName(), m_SlaveFileName.GetFullName());
...
bool tmProjectMerge::MergeIntoMaster()
{
    ...
    if (m_DB->DataBaseQueryNoResults(
            wxString::Format(myQuery, m_MasterFileName.GetFullName(), m_SlaveFileName.GetFullName(),
                             m_MasterFileName.GetFullName()), true) == false) {
        return false;
    }
