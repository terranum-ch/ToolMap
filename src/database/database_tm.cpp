...
bool DataBaseTM::CreateEmptyTMDatabase() {
  ...
  wxString myNewPrjSentence2 =
      ...
      _T("  `PARENT_ID` INT UNSIGNED NOT NULL ,")
      _T("  `TYPE_CD` INT UNSIGNED NOT NULL ,")
...
bool DataBaseTM::SetProjectBackupPath(const wxString &spath) {
  wxString sSentence =
      wxString::Format(_T("UPDATE %s SET PRJ_BACKUP_PATH = \"%s\"; "), TABLE_NAME_PRJ_SETTINGS.c_str(), spath.c_str());
...
int DataBaseTM::GetNextLayer(ProjectDefMemoryLayers *myLayer) {
  wxArrayString myResults;
  wxString sSentence =
      wxString::Format(_T("SELECT LAYER_INDEX, TYPE_CD, LAYER_NAME FROM %s"), TABLE_NAME_LAYERS.c_str());
...
int DataBaseTM::GetLayerID(TOC_GENERIC_NAME layertype) {
  wxString sStatement =
      wxString::Format(_T("SELECT CONTENT_ID FROM ") + TABLE_NAME_TOC + _T(" WHERE GENERIC_LAYERS=%d"), layertype);
...
int DataBaseTM::GetFieldsFromDB(PrjDefMemManage *myPrj) {
  ...
  wxString myQ = wxString::Format(
      _T("select l.ATTRIBUT_ID, l.LAYER_INDEX, l.ATTRIBUT_NAME, c.CATALOG_ID, c.CODE, c.DESCRIPTION_0  FROM %s l LEFT ")
...
bool DataBaseTM::GetFields(PrjMemFieldArray &fieldarray, ProjectDefMemoryLayers *actuallayer) {
  ...
  wxString sSentence =
      wxString::Format(sStatmt, DataBaseGetName().c_str(), TABLE_NAME_LAYER_AT.c_str(), actuallayer->m_LayerID);
  if (!DataBaseQuery(sSentence)) {
    return false;
  }
...
bool DataBaseTM::UpdateField(ProjectDefMemoryFields *myField, int iLayer, wxString &sSqlSentence) {
  ...
  if (IsNewField) {
    ...
    myField->GetStringTypeFromValues(myTypeFieldTemp);
    wxString myModifyQuery =
        wxString::Format(_T("ALTER TABLE %s%d CHANGE COLUMN %s %s %s; "), TABLE_NAME_LAYER_AT.c_str(), iLayer,
                         myField->m_FieldOldName.c_str(), myField->m_Fieldname.c_str(), myTypeFieldTemp.c_str());
...
bool DataBaseTM::DeleteField(wxArrayString &myFields, int iLayer, wxString &sSqlSentence) {
  ...
  for (unsigned int i = myNumberOfItems; i > 0; i--) {
    sSqlSentence.Append(wxString::Format(_T("ALTER TABLE %s%d DROP COLUMN %s; "), TABLE_NAME_LAYER_AT.c_str(), iLayer,
  ...
  for (unsigned int i = myNumberOfItems; i > 0; i--) {
    ...
    // delete from catalog,
    wxString myDeleteTxt =
        wxString::Format(_T("DELETE from %s WHERE CATALOG_ID IN (SELECT CATALOG_ID FROM %s WHERE ATTRIBUT_ID = %ld);"),
...
wxArrayString DataBaseTM::GetLayerNameByType(int ilayertype) {
  ...
  if (ilayertype != LAYER_LINE) {
    sSentence =
        wxString::Format(_T("SELECT LAYER_NAME FROM %s WHERE TYPE_CD = %d"), TABLE_NAME_LAYERS.c_str(), ilayertype);
...
bool DataBaseTM::SetRank(ListGenReport *list, int icol, const wxString &stable, const wxString &sfield,
  ...
  for (int i = 0; i < list->GetItemCount(); i++) {
    ...
    sSentence.Append(wxString::Format(_T("UPDATE %s SET %s = %d WHERE %s = %s; "), stable.c_str(), rankfield.c_str(), i,
                                      sfield.c_str(), myText.c_str()));
  }
...
bool DataBaseTM::SetScaleRank(ScaleList *list, int icol, const wxString &stable, const wxString &sfield,
  ...
  for (int i = 0; i < list->GetItemCount(); i++) {
    ...
    // prepare the sentence
    sSentence.Append(wxString::Format(_T("UPDATE %s SET %s = %d WHERE %s = %ld; "), stable.c_str(), rankfield.c_str(),
                                      i, sfield.c_str(), myScale));
  }
      ...
      if (TableExist(wxString::Format(_T("%s%d"), TABLE_NAME_LAYER_AT.c_str(), pLayers->m_LayerID))) {
        ...
        for (int f = 0; f < pProjDef->GetCountFields(); f++) {
          ...
          for (unsigned int c = 0; c < myField->m_StoreDeleteCodedValues.GetCount(); c++) {
            ...
            wxString myQuery =
                wxString::Format(_T("UPDATE %s%d SET %s = NULL WHERE %s=%ld; "), TABLE_NAME_LAYER_AT,
                                 pLayers->m_LayerID, myField->m_Fieldname, myField->m_Fieldname, myIdToDelete);
...
bool DataBaseTM::DeleteLayerSnapping(int layersid) {
  wxString sSentence =
      wxString::Format(_T("DELETE FROM %s WHERE TOC_ID = %d; "), TABLE_NAME_SNAPPING.c_str(), layersid);
...
bool DataBaseTM::GeometryDelete(wxArrayLong *selected, int layertype) {
  wxString sSentence =
      wxString::Format(_T("DELETE FROM %s WHERE OBJECT_ID IN ("), TABLE_NAME_GIS_GENERIC[layertype].c_str());
...
bool DataBaseTM::DeleteAttribution(wxArrayLong *selected, int layertype) {
  ...
  wxString sSentence =
      wxString::Format(_T("DELETE FROM %s WHERE OBJECT_GEOM_ID IN ("), TABLE_NAME_GIS_ATTRIBUTION[layertype].c_str());
