...
int DataBaseTM::GetFieldsFromDB(PrjDefMemManage *myPrj)
{
    ...
    wxString myQ = wxString::Format(
            _T("select l.ATTRIBUT_ID, l.LAYER_INDEX, l.ATTRIBUT_NAME, c.CATALOG_ID, c.CODE, c.DESCRIPTION_0  FROM %s l LEFT JOIN  (%s c, %s m) ON (l.ATTRIBUT_ID = m.ATTRIBUT_ID AND m.CATALOG_ID = c.CATALOG_ID) ORDER BY l.LAYER_INDEX"),
...
bool DataBaseTM::GetFields(PrjMemFieldArray &fieldarray, ProjectDefMemoryLayers *actuallayer)
{
    ...
    wxASSERT(actuallayer);
    wxString myQ = wxString::Format(
            _T("select l.ATTRIBUT_ID, l.LAYER_INDEX, l.ATTRIBUT_NAME, c.CATALOG_ID, c.CODE, c.DESCRIPTION_0  FROM %s l LEFT JOIN  (%s c, %s m) ON (l.ATTRIBUT_ID = m.ATTRIBUT_ID AND m.CATALOG_ID = c.CATALOG_ID) WHERE l.LAYER_INDEX=%d ORDER BY l.ATTRIBUT_ID"),
...
bool DataBaseTM::DeleteField(wxArrayString &myFields, int iLayer, wxString &sSqlSentence)
{
    ...
    for (unsigned int i = myNumberOfItems; i > 0; i--) {
        ...
        wxString myQuery = wxString::Format(
                _T("SELECT ATTRIBUT_ID FROM %s WHERE LAYER_INDEX=%d AND ATTRIBUT_NAME=\"%s\""), TABLE_NAME_AT_LIST,
                iLayer, myFields.Item(i - 1));
        if (DataBaseQuery(myQuery) == false) {
...
bool DataBaseTM::DeleteFields(int iLayer)
{
    ...
    for (unsigned int c = 0; c < myAttributIDs.GetCount(); c++) {
        ...
        myDeleteTxt.Append(wxString::Format(_T(" DELETE from %s WHERE ATTRIBUT_ID = %ld;"), TABLE_NAME_AT_MIX,
...
void
DataBaseTM::PrepareTOCStatusUpdate(wxString &sentence, tmLayerProperties *item, int itemRank, const wxString &symbology)
{
    wxString myPath = item->GetName().GetPath();
    // converting path to windows path
    // do nothing if not a windows path.
...
bool DataBaseTM::SaveBezierSettings(BezierSettingsData *data)
{
    ...
    wxString myQuery = wxString::Format(
            _T("UPDATE prj_settings SET PRJ_BEZIER_APPROX=%s, PRJ_BEZIER_WIDTH=%s, PRJ_BEZIER_NB_VERTEX=%d, PRJ_BEZIER_METHOD=%d"),
            wxString::FromDouble(data->agg_approximation), wxString::FromDouble(data->ethz_width),
            data->ethz_max_points, (int) data->method);
    if (DataBaseQueryNoResults(myQuery) == false) {
        return false;
    }
    return true;
