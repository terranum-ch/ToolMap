...
bool wxTreeMultiCtrl::Create(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style,
                             const wxValidator& WXUNUSED(validator), const wxString& name) {
...
#if (CHECKBOXVIEW)
    if (_maxHeight < _checkHeight) _maxHeight = _checkHeight;
#endif
...
void wxTreeMultiCtrl::AdjustIconsDeltaY() {
    ...
    if (_captionFont.Ok()) GetTextExtent(wxT("jG"), &x, &y, 0, 0, &_captionFont);
    _captionHeight = y;

    if (_maxHeight < _captionHeight) _maxHeight = _captionHeight;

    // determine the center pos for the [+]
    _iconDeltaY = abs(_maxHeight - _iconHeight) / 2 + 1;
    if (_iconDeltaY < 1) _iconDeltaY = 1;

#if (CHECKBOXVIEW)
    // determine the center pos for the checkbox
    _checkDeltaY = abs(_maxHeight - _checkHeight) / 2 + 1;
    if (_checkDeltaY < 1) _checkDeltaY = 1;
#endif
}
...
wxTreeMultiItem wxTreeMultiCtrl::AppendWindow(const wxTreeMultiItem& ParentItem, wxWindow* window, const wxString& name,
    ...
    wxTreeMultiItem NewWindowItem(
        this->InsertWindow(parent, wx_static_cast(size_t, parent->GetNodeCount()), window, name, info, flags));
    // redraw the stucture:
...
wxTreeMultiItem wxTreeMultiCtrl::InsertWindow(wxTreeMultiItem const& ParentItem, size_t Position, wxWindow* window,
    ...
    // return the new window
    return NewWindowItem;
} /* wxTreeMultiCtrl::InsertWindow(wxTreeMultiItem const&, size_t, wxWindow*, wxString const&, wxTreeMultiWindowInfo
     const&, int) */
...
wxTreeMultiItem wxTreeMultiCtrl::PrependWindow(wxTreeMultiItem const& ParentItem, wxWindow* window,
    ...
    // return the new window
    return NewWindowItem;
} /* wxTreeMultiCtrl::PrependWindow(wxTreeMultiItem const&, wxWindow*, const wxString &, wxTreeMultiWindowInfo const&,
     int) */
...
wxTreeMultiItem wxTreeMultiCtrl::AppendNode(wxTreeMultiItem const& ParentItem, const wxString& caption,
    ...
    // now, append node to the tree control:
    wxTreeMultiItem NewNodeItem(
        this->InsertNode(parent, wx_static_cast(size_t, parent->GetNodeCount()), caption, name));
...
bool wxTreeMultiCtrl::Delete(wxTreeMultiItem& item) {
    ...
    if (ItemIndex != this->GetSelectedItemCount()) this->m_SelectedItems.RemoveAt(ItemIndex);
...
void wxTreeMultiCtrl::DeleteChildren(const wxTreeMultiItem& item) {
    if (item.IsNodeItem()) {
        ...
        if (n->GetNodeCount() > 0) {
            ...
            int Cookie;
            wxTreeMultiItem FirstItemIterator(this->GetFirstChild(item, Cookie)),
                LastItemIterator(this->GetLastChild(item));
...
void wxTreeMultiCtrl::ExpandNodes(bool recursive) {
    ...
    for (int i = 0; i < _root.GetNodeCount(); i++) DoFold(_root.GetNode(i), true, recursive);
    RedrawFromNode(0);
}

void wxTreeMultiCtrl::CollapseNodes(bool recursive) {
    // go through all children and call DoFold recursively
    for (int i = 0; i < _root.GetNodeCount(); i++) DoFold(_root.GetNode(i), false, recursive);
    RedrawFromNode(0);
}
...
size_t wxTreeMultiCtrl::GetSelectedItemIndex(wxTreeMultiItem const& Item) const {
    // attention: the function wxArray::Index() can NOT be used in a save manner as it only checks the address of an
    // item
    ...
    while ((Index < NoOfSelectedItems) && (this->m_SelectedItems[Index] != Item)) ++Index;
    return Index;
...
void wxTreeMultiCtrl::SelectItem(wxTreeMultiItem const& Item, bool UnselectOthers, bool ExpandSelection) {
    ...
    if (this->GetEventHandler()->ProcessEvent(Event) && !(Event.IsAllowed())) return;  // vetoed
    ...
    if (UnselectOthers) this->UnselectAll();
    ...
    if (ExpandSelection) {
        ...
        for (;;) {
            ...
            if (FirstItemIterator == LastItemIterator) break;  // done
            // continue iterating:
            FirstItemIterator = this->GetNext(FirstItemIterator);
        } /* for */
    }     /* if */
...
wxTreeMultiItem wxTreeMultiCtrl::GetExcludedParent(const wxTreeMultiItem& item) {
    ...
    TreeMultiItemNode* n = item.GetItem()->IsTreeMultiItemNode();
    if (n && n->IsExcluded()) return wxTreeMultiItem(n);
...
void wxTreeMultiCtrl::OnPaint(wxPaintEvent& WXUNUSED(event)) {
    ...
    for (int i = 0; i < _root.GetNodeCount(); i++) DrawNode(_root.GetNode(i), dc);
}
...
void wxTreeMultiCtrl::DrawNode(TreeMultiItemBase* b, wxDC& dc) {
    ...
    if (b->IsVisible()) {
        ...
        if (b->IsTreeMultiItemNode()) {
            ...
            for (int i = 0; i < n->GetNodeCount(); i++) DrawNode(n->GetNode(i), dc);
        }
    }
}
...
void wxTreeMultiCtrl::DrawCheckbox(TreeMultiItemBase* b, wxDC& dc, bool convertScrolled) {
    ...
    if (convertScrolled) CalcScrolledPosition(x, y, &xx, &yy);
...
void wxTreeMultiCtrl::OnKey(wxKeyEvent& event) {
    ...
    if (event.GetKeyCode() == WXK_TAB) {
        ...
        if (item.IsOk()) {
            ...
            if (!event.ShiftDown()) item = FindNextVisibleWindowItem(item.GetItem());
                ...
                if (event.LeftDown()) {
                    if (flags == wxTMC_HITTEST_GUTTER) {
                        ...
                        if (flags == wxTMC_HITTEST_CAPTION) {
                            TreeMultiItemNode* n =
                                id.GetItem()
                                    ->IsTreeMultiItemNode();  // for some reasons also windows may have set the flag
...
wxTreeMultiItem wxTreeMultiCtrl::HitTest(wxPoint const& pt, int& flags) {
    ...
    for (int i = 0; i < _root.GetNodeCount() && !b; i++) b = FindNodeByPoint(_root.GetNode(i), pt, flags);
...
TreeMultiItemBase* wxTreeMultiCtrl::FindNodeByPoint(TreeMultiItemBase* b, wxPoint const& pt, int& area) {
    ...
    if (pt.y >= b->GetY() && pt.y < (b->GetY() + b->GetHeight())) {
        ...
        if (pt.x < b->GetX()) area = wxTMC_HITTEST_GUTTER;
        ...
        if (n) {
            ...
            for (int i = 0; i < n->GetNodeCount() && !bb; i++) bb = FindNodeByPoint(n->GetNode(i), pt, area);
...
wxTreeMultiItem wxTreeMultiCtrl::GetFocus() {
    ...
    if (item.IsOk() && item.GetItem()->IsVisible()) return item;
...
void wxTreeMultiCtrl::SetRecursiveCheckState(TreeMultiItemNode* n, bool check) {
    ...
    for (int i = 0; i < n->GetNodeCount(); i++) {
        ...
        if (n->GetNode(i)->IsTreeMultiItemNode()) SetRecursiveCheckState((TreeMultiItemNode*)n->GetNode(i), check);
    }
}
    ...
    if (p && p->GetCheckbox()) {
        ...
        for (size_t i = 0; i < (size_t)p->GetNodeCount(); ++i) {
            ...
            if (p->GetNode(i)->IsTreeMultiItemWindow() && p->GetNode(i)->GetCheckbox()) {
                ...
                if (!p->GetNode(i)->GetCheckboxState()) foundclear = true;
                // record instance of checked checkbox
                if (p->GetNode(i)->GetCheckboxState() == 1) foundcheck = true;
            }
        }
...
wxTreeMultiItem wxTreeMultiCtrl::InsertWindow(TreeMultiItemNode* ParentPtr, size_t Position, wxWindow* WindowPtr,
    ...
    // get flags from passed variable "Flags"; in case this variable does not contain any flags use the window's
    // information flags:
    if (Flags != 0)
        WindowFlags = Flags;
    else
...
} /* wxTreeMultiCtrl::InsertWindow(TreeMultiItemNode*, size_t, wxWindow*, wxString const&, wxTreeMultiWindowInfo const&,
     int) */
...
void wxTreeMultiCtrl::RecalculateNodePositions() {
    ...
    for (int i = 0; i < _root.GetNodeCount(); i++) currentY += CalculateNodeDimensions(_root.GetNode(i), currentY, 0);
}
...
#if (CHECKBOXVIEW)
    if (b->GetCheckbox()) gutter += _checkWidth;
#endif
...
void wxTreeMultiCtrl::RecalculateSpanSizes() {
    for (int i = 0; i < _root.GetNodeCount(); i++) CalculateNodeSpanning(_root.GetNode(i));
}
...
void wxTreeMultiCtrl::CalculateNodeSpanning(TreeMultiItemBase* b) {
    ...
    if (b->IsTreeMultiItemNode()) {
        ...
        for (int i = 0; i < n->GetNodeCount(); i++) CalculateNodeSpanning(n->GetNode(i));
    } else if (b->IsTreeMultiItemWindow()) {
        TreeMultiItemWindow* w = (TreeMultiItemWindow*)b;
        wxWindow* wnd = w->GetWindow();
        if (wnd) {
            ...
            if (w->GetHorizontalSpan()) {
                ...
                if (sz) {
                    if (maxwidth < sz->GetMinSize().GetWidth()) maxwidth = sz->GetMinSize().GetWidth();
                }
                ...
                if (wnd->GetSizer()) wnd->GetSizer()->Layout();
            }
        }
    }
}
...
void wxTreeMultiCtrl::AdjustScrollbars(int x, int y) {
    ...
    SetScrollbars(WXTMC_PIXELS_PER_UNIT, WXTMC_PIXELS_PER_UNIT, x / WXTMC_PIXELS_PER_UNIT, y / WXTMC_PIXELS_PER_UNIT,
                  x_pos, y_pos, true);
}
...
void wxTreeMultiCtrl::RecalculateVirtualSizeFromNode(const TreeMultiItemNode* node, int& x, int& y) {
    ...
    if (x < (node->GetWidth() + node->GetX())) x = node->GetWidth() + node->GetX();
    ...
    if (node->IsExpanded()) {
        ...
        for (int i = 0; i < node->GetNodeCount(); i++) {
            ...
            // calculate x and y
            if (x < (b->GetWidth() + b->GetX())) x = b->GetWidth() + b->GetX();

            y = b->GetY() + b->GetHeight();

            if (b->IsTreeMultiItemNode()) RecalculateVirtualSizeFromNode((TreeMultiItemNode*)b, x, y);
        }
    }
}
...
void wxTreeMultiCtrl::SetWindowBackgroundColour(wxWindow* wnd, const wxColour& col, int flags) {
    if (wnd) {
        ...
        if (!btn || ((flags & wxTMC_BG_ADJUST_BTN) != 0)) wnd->SetBackgroundColour(col);
...
void wxTreeMultiCtrl::ShowTreeMultiWindow(TreeMultiItemWindow* window, bool show) {
    // show or hide window
    if (window && window->GetWindow()) window->GetWindow()->Show(show);
}

void wxTreeMultiCtrl::UpdateAllWindowVisibility() {
    // all roots are visible, but what lies beneath ... who knows
    for (int i = 0; i < _root.GetNodeCount(); i++) UpdateTreeMultiWindowVisibility(_root.GetNode(i), true);
}
...
void wxTreeMultiCtrl::UpdateTreeMultiWindowVisibility(TreeMultiItemBase* b, bool show) {
    if (b) {
        ...
        if (b->IsTreeMultiItemWindow()) {
            ...
            for (int i = 0; i < node->GetNodeCount(); i++) UpdateTreeMultiWindowVisibility(node->GetNode(i), showMe);
        }
    }
}
...
wxTreeMultiItem wxTreeMultiCtrl::FindItem(const wxTreeMultiItem& item, const wxString& name, bool ignoreCase,
    ...
    if (item.IsOk()) {
        ...
        if (!skipFirst) {
            if (b->GetName().IsSameAs(name, !ignoreCase)) return wxTreeMultiItem(b);
        }
...
wxTreeMultiItem wxTreeMultiCtrl::FindWindowNode(wxWindow* wnd, TreeMultiItemNode* n) {
    ...
    if (!n) n = (TreeMultiItemNode*)&_root;
    ...
    for (int i = 0; i < n->GetNodeCount() && !result.IsOk(); i++) {
        ...
        if (w && w->GetWindow() == wnd) return wxTreeMultiItem(n);

        // if node, go deeper
        if (n->GetNode(i)->IsTreeMultiItemNode()) result = FindWindowNode(wnd, (TreeMultiItemNode*)n->GetNode(i));
    }
    ...
    if (b->IsVisible()) {
        ...
        if (n) {
            for (int i = index + 1; i < n->GetNodeCount() && !value; i++) {
                ...
                if (!value) value = FindNextVisibleWindowItem(bn, -1);
            }
...
wxString wxTreeMultiCtrl::GetTextValue(int wndId) {
    ...
    if (c1) return c1->GetStringSelection();

    // try a combo box
    wxComboBox* c2 = wxDynamicCast(wnd, wxComboBox);
    if (c2) return c2->GetStringSelection();

    // try a listbox
    wxListBox* l = wxDynamicCast(wnd, wxListBox);
    if (l) return l->GetStringSelection();
...
int wxTreeMultiCtrl::GetSelectionValue(int wndId) {
    ...
    // try a combo box
    wxComboBox* c2 = wxDynamicCast(wnd, wxComboBox);
    if (c2) return c2->GetSelection();

    // try a listbox
    wxListBox* l = wxDynamicCast(wnd, wxListBox);
    if (l) return l->GetSelection();
...
wxTreeMultiItem wxTreeMultiCtrl::GetLastChild(const wxTreeMultiItem& item) const {
    if (item.IsNodeItem()) {
        ...
        if (n->GetNodeCount() > 0) return wxTreeMultiItem(n->GetNode(n->GetNodeCount() - 1));
    }
...
wxTreeMultiItem wxTreeMultiCtrl::GetNextSibling(wxTreeMultiItem const& item) const {
    ...
    if (!(item.IsOk())) return wxTreeMultiItem();
    ...
    {
        // find the current item in the parent's list; the next sibling has an index that is one higher than the one of
        // the current item:
