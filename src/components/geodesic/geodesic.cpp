...
static void Lengths(const struct geod_geodesic* g, real eps, real sig12, real ssig1, real csig1, real dn1, real ssig2,
                    real csig2, real dn2, real cbet1, real cbet2, real* ps12b, real* pm12b, real* pm0, boolx scalep,
                    real* pM12, real* pM21,
                    /* Scratch areas of the right size */
                    real C1a[], real C2a[]);
static real Astroid(real x, real y);
static real InverseStart(const struct geod_geodesic* g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2,
                         real dn2, real lam12, real* psalp1, real* pcalp1,
...
static real Lambda12(const struct geod_geodesic* g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2, real dn2,
                     real salp1, real calp1, real* psalp2, real* pcalp2, real* psig12, real* pssig1, real* pcsig1,
                     real* pssig2, real* pcsig2, real* peps, real* pdomg12, boolx diffp, real* pdlam12,
                     /* Scratch areas of the right size */
...
void geod_init(struct geod_geodesic* g, real a, real f) {
    ...
    g->b = g->a * g->f1;
    g->c2 = (sq(g->a) +
             sq(g->b) * (g->e2 == 0 ? 1 : (g->e2 > 0 ? atanhx(sqrt(g->e2)) : atan(sqrt(-g->e2))) / sqrt(fabs(g->e2)))) /
...
void geod_lineinit(struct geod_geodesicline* l, const struct geod_geodesic* g, real lat1, real lon1, real azi1,
                   unsigned caps) {
    ...
    l->caps = (caps ? caps : GEOD_DISTANCE_IN | GEOD_LONGITUDE) | GEOD_LATITUDE |
              GEOD_AZIMUTH; /* Always allow latitude and azimuth */
...
real geod_genposition(const struct geod_geodesicline* l, boolx arcmode, real s12_a12, real* plat2, real* plon2,
                      real* pazi2, real* ps12, real* pm12, real* pM12, real* pM21, real* pS12) {
    ...
    if (outmask & (GEOD_DISTANCE | GEOD_REDUCEDLENGTH | GEOD_GEODESICSCALE)) {
        if (arcmode || fabs(l->f) > 0.01) B12 = SinCosSeries(TRUE, ssig2, csig2, l->C1a, nC1);
    ...
    if (outmask & GEOD_DISTANCE) s12 = arcmode ? l->b * ((1 + l->A1m1) * sig12 + AB1) : s12_a12;
    ...
    if (outmask & GEOD_LATITUDE) lat2 = atan2(sbet2, l->f1 * cbet2) / degree;

    if (outmask & GEOD_AZIMUTH) /* minus signs give range [-180, 180). 0- converts -0 to +0. */
...
real geod_gendirect(const struct geod_geodesic* g, real lat1, real lon1, real azi1, boolx arcmode, real s12_a12,
                    real* plat2, real* plon2, real* pazi2, real* ps12, real* pm12, real* pM12, real* pM21, real* pS12) {
...
real geod_geninverse(const struct geod_geodesic* g, real lat1, real lon1, real lat2, real lon2, real* ps12, real* pazi1,
    ...
    unsigned outmask = (ps12 ? GEOD_DISTANCE : 0U) | (pazi1 || pazi2 ? GEOD_AZIMUTH : 0U) |
                       (pm12 ? GEOD_REDUCEDLENGTH : 0U) | (pM12 || pM21 ? GEOD_GEODESICSCALE : 0U) |
                       (pS12 ? GEOD_AREA : 0U);
    ...
    if (!meridian && sbet1 == 0 && /* and sbet2 == 0 */
        ...
        real dnm = 0;
        sig12 = InverseStart(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, &salp1, &calp1, &salp2, &calp2, &dnm, C1a,
                             C2a);
        ...
        if (sig12 >= 0) {
            ...
            for (tripn = FALSE, tripb = FALSE; numit < maxit2; ++numit) {
                ...
                real dv,
                    v = (Lambda12(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1, calp1, &salp2, &calp2, &sig12, &ssig1,
...
void Lengths(const struct geod_geodesic* g, real eps, real sig12, real ssig1, real csig1, real dn1, real ssig2,
             real csig2, real dn2, real cbet1, real cbet2, real* ps12b, real* pm12b, real* pm0, boolx scalep,
             real* pM12, real* pM21,
             /* Scratch areas of the right size */
             real C1a[], real C2a[]) {
...
real InverseStart(const struct geod_geodesic* g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2, real dn2,
    ...
    if (shortline && ssig12 < g->etol2) {
        ...
        if (g->f >= 0) { /* In fact f == 0 does not get here */
             ...
             * Inverse. */
            Lengths(g, g->n, pi + bet12a, sbet1, -cbet1, dn1, sbet2, cbet2, dn2, cbet1, cbet2, &dummy, &m12b, &m0,
...
real Lambda12(const struct geod_geodesic* g, real sbet1, real cbet1, real dn1, real sbet2, real cbet2, real dn2,
              real salp1, real calp1, real* psalp2, real* pcalp2, real* psig12, real* pssig1, real* pcsig1,
              real* pssig2, real* pcsig2, real* peps, real* pdomg12, boolx diffp, real* pdlam12,
              /* Scratch areas of the right size */
              real C1a[], real C2a[], real C3a[]) {
    ...
    if (diffp) {
        if (calp2 == 0)
            ...
            real dummy;
            Lengths(g, eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2, &dummy, &dlam12, &dummy, FALSE,
                    &dummy, &dummy, C1a, C2a);
...
void C3f(const struct geod_geodesic* g, real eps, real c[]) {
    ...
    for (j = nC3x, k = nC3 - 1; k;) {
        real t = 0;
        for (i = nC3 - k; i; --i) t = eps * t + g->C3x[--j];
        c[k--] = t;
    }
...
void C4f(const struct geod_geodesic* g, real eps, real c[]) {
    ...
    for (j = nC4x, k = nC4; k;) {
        real t = 0;
        for (i = nC4 - k + 1; i; --i) t = eps * t + g->C4x[--j];
        c[--k] = t;
    }
...
unsigned geod_polygon_compute(const struct geod_geodesic* g, const struct geod_polygon* p, boolx reverse, boolx sign,
                              real* pA, real* pP) {
...
unsigned geod_polygon_testpoint(const struct geod_geodesic* g, const struct geod_polygon* p, real lat, real lon,
    ...
    for (i = 0; i < (p->polyline ? 1 : 2); ++i) {
        real s12, S12;
        geod_geninverse(g, i == 0 ? p->lat : lat, i == 0 ? p->lon : lon, i != 0 ? p->lat0 : lat, i != 0 ? p->lon0 : lon,
...
unsigned geod_polygon_testedge(const struct geod_geodesic* g, const struct geod_polygon* p, real azi, real s,
                               boolx reverse, boolx sign, real* pA, real* pP) {
    real perimeter, tempsum, area0;
    int crossings;
    unsigned num = p->num + 1;
