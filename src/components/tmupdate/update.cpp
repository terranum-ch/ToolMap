...
void WebUpdateInformationBar::OnNewVersion(wxThreadEvent& event) {
    ...
    wxString myMessage = wxString::Format(_("New version of %s is available: %s (%d)"), g_ProgName, event.GetString(),
                                          event.GetInt());
...
void WebUpdateInformationBar::OnNoNewVersion(wxThreadEvent& event) {
    _ClearBar();
    wxString myMessage = wxString::Format(_("No new version of %s is available!"), g_ProgName);
    ShowMessage(myMessage);
}

WebUpdateInformationBar::WebUpdateInformationBar(wxWindow* parent)
    : wxInfoBar(parent, wxID_ANY) {
    m_Parent = parent;
}
...
WebUpdateThread::WebUpdateThread(wxWindow* parent, const wxString& proxy) {
    ...
    myCurlError = curl_easy_setopt(m_CurlHandle, CURLOPT_URL, (const char*)myServerInfo.mb_str(wxConvUTF8));
    wxASSERT(myCurlError == CURLE_OK);
...
bool WebUpdateThread::CheckNewVersion(long actualversion, bool msgNewVersion, bool msgNoNewVersion,
                                      bool msgNoInternet) {
