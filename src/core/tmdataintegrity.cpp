...
tmDataIntegrity::tmDataIntegrity(DataBaseTM * database) {
	...
	m_QueryTemplateType = _T(" FROM %s AS mix WHERE mix.OBJECT_GEOM_ID NOT IN (SELECT OBJECT_ID FROM %s)");
...
bool tmDataIntegrity::CheckType(const TOC_GENERIC_NAME & layertype) {
	wxASSERT(m_pDB);
	m_ErrorType = wxNOT_FOUND;
	m_ErrorAAttrib = wxNOT_FOUND;
...
bool tmDataIntegrity::CheckAAttrib(long layerindex, const TOC_GENERIC_NAME & layertype) {
	wxASSERT(m_pDB);
	m_ErrorType = wxNOT_FOUND;
	m_ErrorAAttrib = wxNOT_FOUND;
...
bool tmDataIntegrity::CorrectType(long layerindex, const TOC_GENERIC_NAME & layertype) {
	return false;
}
