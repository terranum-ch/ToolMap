...
tmAAttribBatchManager::tmAAttribBatchManager(PrjDefMemManage *project, DataBaseTM *database,
                                             tmSelectedDataMemory *selected, PRJDEF_LAYERS_TYPE sellayertype) {
  m_Project = project;
  m_DB = database;
  m_Selected = selected;
...
bool tmAAttribBatchManager::_GetSelectionSubset(long layerid, wxArrayLong &subselected) {
  ...
  wxString myQuery = wxString::Format(
      _T("SELECT a.OBJECT_GEOM_ID FROM %s a LEFT JOIN %s o")
...
wxString tmAAttribBatchManager::_CreateListOfIds(const wxArrayLong *ids) {
  ...
  for (unsigned int i = 0; i < ids->GetCount(); i++) {
    myIdsText.Append(wxString::Format(_T("%ld,"), ids->Item(i)));
  }
...
int tmAAttribBatchManager::Attribute(long layerid, const ProjectDefMemoryFields &field, const wxString &value) {
  ...
  for (unsigned int i = 0; i < mySubSet.GetCount(); i++) {
    myQuery.Append(
        wxString::Format(_T("INSERT INTO %s%ld (OBJECT_ID, %s) VALUES (%ld, \"%s\")")
                         _T(" ON DUPLICATE KEY UPDATE %s=\"%s\";"),
                         TABLE_NAME_LAYER_AT.c_str(), layerid, field.m_Fieldname.c_str(), mySubSet.Item(i),
