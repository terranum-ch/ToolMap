...
bool tmProjectUpdater::_221to222()
{
    ...
    for (unsigned int f = 0; f < myFields.GetCount(); f++) {
        ...
        for (unsigned int e = 0; e < myF->m_pCodedValueArray.GetCount(); e++) {
            ...
            if (m_pDB->DataBaseQueryNoResults(
                    wxString::Format(myQuery, TABLE_NAME_LAYER_AT, myLayerIndex.Item(f), myF->m_Fieldname,
                                     myCVal->m_ValueID, myF->m_Fieldname, myCVal->m_ValueName)) == false) {
        ...
        if (m_pDB->DataBaseQueryNoResults(
                wxString::Format(myQuery, TABLE_NAME_LAYER_AT, myLayerIndex.Item(f), myF->m_Fieldname,
                                 TABLE_COMMENT_ENUMERATION)) == false) {
...
bool tmProjectUpdater::_225to226()
{
    ...
    myAlterQuery.Append(
            _T(" ALTER TABLE prj_settings ADD COLUMN  `PRJ_BEZIER_APPROX` FLOAT NOT NULL DEFAULT 0.5 AFTER `PRJ_LANG_ACTIVE`;"));
