...
bool tmProjectUpdater::_SetVersion(int versionnumber) {
    wxString myQuery = wxString::Format(_T("UPDATE `%s` SET PRJ_VERSION = %d"), TABLE_NAME_PRJ_SETTINGS, versionnumber);
...
bool tmProjectUpdater::_220to221() {
    ...
    wxString myQuery =
        _T("CREATE  TABLE IF NOT EXISTS `prj_stats` (`STAT_ID` INT NOT NULL AUTO_INCREMENT ,")
...
bool tmProjectUpdater::_221to222() {
    ...
    for (unsigned int f = 0; f < myFields.GetCount(); f++) {
        ...
        if (myF->m_FieldID == wxNOT_FOUND) {
            wxLogError(_("Unable to get ID for newly inserted field '%s', failed to convert project"),
                       myF->m_Fieldname);
            continue;
        }
        ...
        for (unsigned int e = 0; e < myF->m_pCodedValueArray.GetCount(); e++) {
            ...
            if (!m_pDB->DataBaseQueryNoResults(
                ...
                wxLogError(_("Adding ennumeration: '%s' from layer '%s' failed"), myCVal->m_ValueName,
                           myF->m_Fieldname);
                continue;
            }
            ...
            if (myCVal->m_ValueID == wxNOT_FOUND) {
                wxLogError(_("Unable to get ID for newly inserted enumeration '%s' from layer '%s'"),
                           myCVal->m_ValueName, myF->m_Fieldname);
                continue;
            }
    ...
    for (unsigned int f = 0; f < myFields.GetCount(); f++) {
        ...
        if (m_pDB->DataBaseQueryNoResults(
                wxString::Format(myQuery, TABLE_NAME_LAYER_AT, myLayerIndex[f], myF->m_Fieldname), true) == false) {
    ...
    for (unsigned int f = 0; f < myFields.GetCount(); f++) {
        ...
        for (unsigned int e = 0; e < myF->m_pCodedValueArray.GetCount(); e++) {
            ...
            if (m_pDB->DataBaseQueryNoResults(wxString::Format(myQuery, TABLE_NAME_LAYER_AT, myLayerIndex.Item(f),
                                                               myF->m_Fieldname, myCVal->m_ValueID, myF->m_Fieldname,
                                                               myCVal->m_ValueName)) == false) {
                wxLogError(_("Error setting enumeration value from '%s' to '%ld'"), myCVal->m_ValueName,
                           myCVal->m_ValueID);
                continue;
            }
        }
        ...
        if (m_pDB->DataBaseQueryNoResults(wxString::Format(myQuery, TABLE_NAME_LAYER_AT, myLayerIndex.Item(f),
                                                           myF->m_Fieldname, TABLE_COMMENT_ENUMERATION)) == false) {
...
bool tmProjectUpdater::_225to226() {
    ...
    wxString myQuery =
        _T(" SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = \"%s\" AND TABLE_NAME = 'prj_settings' AND ")
        _T("COLUMN_NAME = 'PRJ_LANG_ACTIVE'");
...
bool tmProjectUpdater::_226to227() {
    wxString myAlterQuery =
        _T("ALTER TABLE prj_settings ADD COLUMN `PRJ_BEZIER_WIDTH` FLOAT NOT NULL DEFAULT 1 AFTER ")
        _T("`PRJ_BEZIER_APPROX`; ")
...
bool tmProjectUpdater::_229to230() {
    wxString myQuery =
        _T("ALTER TABLE prj_toc ADD COLUMN `LABEL_VISIBLE` tinyint(1) DEFAULT 0; ")
...
bool tmProjectUpdater::_230to231() {
    wxString myQuery =
        _T("ALTER TABLE prj_settings ADD COLUMN `PRJ_LAST_EXPORTED` TEXT NULL AFTER `PRJ_EXPORT_TYPE`; ");
    return m_pDB->DataBaseQueryNoResults(myQuery);
}
...
bool tmProjectUpdater::_232to233() {
    wxString myQuery = _T("ALTER TABLE prj_toc ADD COLUMN `PARENT_ID` INT UNSIGNED DEFAULT 0; ");
    return m_pDB->DataBaseQueryNoResults(myQuery);
}