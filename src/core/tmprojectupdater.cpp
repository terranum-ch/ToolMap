...
bool tmProjectUpdater::_221to222() {
  ...
  for (unsigned int f = 0; f < myFields.GetCount(); f++) {
    ...
    for (unsigned int e = 0; e < myF->m_pCodedValueArray.GetCount(); e++) {
      ...
      if (myCVal->m_ValueID == wxNOT_FOUND) {
        wxLogError(_("Unable to get ID for newly inserted enumeration '%s' from layer '%s'"), myCVal->m_ValueName,
                   myF->m_Fieldname);
        continue;
      }
  ...
  for (unsigned int f = 0; f < myFields.GetCount(); f++) {
    ...
    if (m_pDB->DataBaseQueryNoResults(wxString::Format(myQuery, TABLE_NAME_LAYER_AT, myLayerIndex[f], myF->m_Fieldname),
                                      true) == false) {
