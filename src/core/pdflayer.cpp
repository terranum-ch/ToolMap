...
void PdfLayer::_GenerateObjects() {
  ...
  mypPdf->Ln();
  mypPdf->Cell(m_ColWidthObjects.Item(0) + m_ColWidthObjects.Item(1), myLineNormal, _("Objects"), wxPDF_BORDER_FRAME, 1,
               wxPDF_ALIGN_CENTER);
  ...
  switch (field->m_FieldType) {
    ...
    case TM_FIELD_ENUMERATION: {
      ...
      for (unsigned int i = 0; i < field->m_pCodedValueArray.GetCount(); i++) {
        ...
        wxString myAttributText = wxString::Format(_T("%s  (%s)"), field->m_pCodedValueArray.Item(i)->m_ValueName,
                                                   field->m_pCodedValueArray.Item(i)->m_ValueCode);
        if (i == 0) {
          mypPdf->Cell(m_ColWidthAttributs.Item(1), myLineSmall, PRJDEF_FIELD_TYPE_STRING[TM_FIELD_ENUMERATION],
                       wxPDF_BORDER_LEFT | wxPDF_BORDER_RIGHT | wxPDF_BORDER_TOP, 0, wxPDF_ALIGN_LEFT);
          mypPdf->Cell(m_ColWidthAttributs.Item(2), myLineSmall, myAttributText, wxPDF_BORDER_RIGHT | wxPDF_BORDER_TOP,
                       0, wxPDF_ALIGN_LEFT, fill);

        } else {
          mypPdf->Cell(m_ColWidthAttributs.Item(1), myLineSmall, "", wxPDF_BORDER_LEFT, 0);
          mypPdf->Cell(m_ColWidthAttributs.Item(2), myLineSmall, myAttributText, wxPDF_BORDER_LEFT | wxPDF_BORDER_RIGHT,
                       0, wxPDF_ALIGN_LEFT, fill);
        }
    ...
    default:
      mypPdf->Cell(m_ColWidthAttributs.Item(1), myLineSmall, "NOT SUPPORTED NOW!", wxPDF_BORDER_LEFT | wxPDF_BORDER_TOP,
                   0);
...
double PdfLayer::GetAttributsWidth(wxPdfDocument *pdf) {
  ...
  for (unsigned int f = 0; f < m_prjLayer->m_pLayerFieldArray.GetCount(); f++) {
    ...
    switch (myField->m_FieldType) {
      ...
      case TM_FIELD_ENUMERATION:
        maxFieldTypeWidth =
            MAX(maxFieldTypeWidth, pdf->GetStringWidth(PRJDEF_FIELD_TYPE_STRING[myField->m_FieldType] + _T("  ")));
        break;

      case TM_FIELD_TEXT: {
        wxString myDummyText = wxString::Format(_("%s (max: %d characters)     "),
                                                PRJDEF_FIELD_TYPE_STRING[TM_FIELD_TEXT], myField->m_FieldPrecision);
