...
bool tmStatsRecords::Load(long recordid, tmStatsData &data) {
  wxASSERT(m_pDB);
  wxString myQuery = wxString::Format(
      _T("SELECT DATE_START, CLICK, ATTRIBUTION, INTERSECTION,")
  ...
  data.m_TimeElapsed = wxTimeSpan(myTempDTime.GetHour(), myTempDTime.GetMinute(), myTempDTime.GetSecond());
  return true;
}
...
bool tmStatsRecords::LoadTotal(tmStatsData &data) {
  wxASSERT(m_pDB);
  wxString myQuery = wxString::Format(
      _T("SELECT SUM(CLICK), SUM(ATTRIBUTION), SUM(INTERSECTION),")
