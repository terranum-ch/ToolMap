...
bool tmExportDataSHP::AddConcatAttributs(ProjectDefMemoryLayers *layer, PrjDefMemManage *projdef, long loop,
  ...
  if (m_Shp->GetCount() != loop) {
    wxLogError(_("Not the number of features expected in '%s' (expected: %ld, found: %ld"), layer->m_LayerName, loop,
               m_Shp->GetCount());
    return false;
  }
  ...
  for (long i = 0; i < loop; i++) {
    ...
    for (unsigned int a = 0; a < myNbAttrib; a++) {
      ...
      wxString myQuery = wxString::Format(
          _T("SELECT %s FROM %s l  JOIN (%s AS a, %s AS o, layer_at%ld AS at) ON (l.OBJECT_ID = a.OBJECT_GEOM_ID AND ")
          _T("a.OBJECT_VAL_ID = o.OBJECT_ID AND at.OBJECT_ID = l.OBJECT_ID) WHERE l.OBJECT_ID = %ld AND o.OBJECT_CD = ")
          _T("\"%s\""),
...
bool tmExportDataSHP::SetPolyExportInfo(ProjectDefMemoryLayers *layer) {
  wxASSERT(m_pDB);
  wxString myQuery =
      wxString::Format(_T("REPLACE INTO %s VALUES (%d,%f,%d,%f)"), TABLE_NAME_EXPORT_POLY, layer->m_LayerID,
                       m_ExportPolyRasterFactor, m_ExportPolyNbIteration, m_ExportPolyPercentSkipped);
