...
class pod_array {
 public:
  ...
  pod_array(unsigned size) : m_array(pod_allocator<T>::allocate(size)), m_size(size) {}

  pod_array(const self_type &v) : m_array(pod_allocator<T>::allocate(v.m_size)), m_size(v.m_size) {
...
class range_adaptor {
 public:
  ...
  range_adaptor(Array &array, unsigned start, unsigned size) : m_array(array), m_start(start), m_size(size) {}
