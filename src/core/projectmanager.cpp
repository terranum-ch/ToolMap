...
bool ProjectManager::CreateNewProject() {
    ...
    if (OpenProject(PrjDefinition.m_PrjPath + wxFileName::GetPathSeparator() + PrjDefinition.m_PrjName) !=
        tmDB_OPEN_OK) {
        // m_pMManager->SetStatus(MENU_DB_CLOSED);
        CloseProject();
        return false;
...
bool ProjectManager::PMAddDefaultQueries() {
    ...
    wxString myQueryTemplate =
        _T("SELECT o.OBJECT_ID FROM %s AS o ")
...
bool ProjectManager::BackupProject(const wxString& backup_comment) {
    ...
    myBckFile.SetInputDirectory(wxFileName(GetDatabase()->DataBaseGetPath(), GetDatabase()->DataBaseGetName()));
    myBckFile.SetDate(wxDateTime::Now());
    myBckFile.SetOutputName(wxFileName(myBackupPath, GetDatabase()->DataBaseGetName(), "tmbk"));
...
bool ProjectManager::MergeProjects(const wxString& slave_project_name, bool beVerbose) {
    ...
    if (wxDirExists(tmpPath)) {
        if (!CleanDirectory(tmpPath)) return false;
    }
...
bool ProjectManager::IsDataBasePath(const wxString& path) {
    // upper case
    if (!wxDir::FindFirst(path, TABLE_NAME_PRJ_SETTINGS + _T(".MYD"), wxDIR_FILES).IsEmpty()) return TRUE;

    // lower case
    if (!wxDir::FindFirst(path, TABLE_NAME_PRJ_SETTINGS.Lower() + _T(".MYD"), wxDIR_FILES).IsEmpty()) return TRUE;

    wxLogDebug(_T("Not a ToolMap project, file : %s not found in %s"), (TABLE_NAME_PRJ_SETTINGS + _T(".MYD")).c_str(),
               path.c_str());
    return FALSE;
}
...
bool ProjectManager::EditObjectFrequency(int id) {
    ...
    wxString sSelect = wxString::Format(
        _T("SELECT OBJECT_ID, OBJECT_ISFREQ FROM %s ")
        _T(" WHERE OBJECT_ID = %d"),
        TABLE_NAME_OBJECTS.c_str(), id);

    if (!m_DB->DataBaseQuery(sSelect)) return false;

    if (!m_DB->DataBaseHasResults()) return false;
    ...
    wxString sUpdate = wxString::Format(
        ...
        _T("WHERE OBJECT_ID = %d"),
        TABLE_NAME_OBJECTS.c_str(), lFreq, id);
