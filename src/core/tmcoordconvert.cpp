...
wxBitmap *tmCoordConvert::GetProjectGoogleRaster(wxBitmap *web_raster, tmRealRect *coord_web, tmRealRect *coord_local) {
  ...
  for (int i = 1; i <= 3; i++) {
    ...
    if (0 <= offs && offs < 3) {
      CPLErr ret = GDALRasterIO(hband, GF_Write, 0, 0, web_raster->GetWidth(), web_raster->GetHeight(),
                                myImgData + offs, web_raster->GetWidth(), web_raster->GetHeight(), GDT_Byte, 3, 0);
  ...
  CPLErr myErr = GDALCreateAndReprojectImage(hOriginDS, pSpatialWKT, myDestName.GetFullPath().mb_str(), pSpatialDestWKT,
                                             hDriver, NULL, GRA_Bilinear, 0.0, 0.0, NULL, NULL, NULL);
  if (myErr != CE_None) {
  ...
  wxFileName myDestName (wxStandardPaths::Get().GetAppDocumentsDir(), _T("test_dest.tif"));
  hDestDS = GDALCreate( hDriver, myDestName.GetFullPath().mb_str(), web_raster->GetWidth(), web_raster->GetHeight(), 3,
