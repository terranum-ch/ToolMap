...
bool TextParser::CheckParseFileExist() {
  ...
  if (wxFile::Exists(m_ParseFileName.GetFullPath())) return TRUE;
  return FALSE;
}
...
wxString TextParser::GetAllSupportedParserWildCards() {
  ...
  for (unsigned int i = 0; i < (sizeof(TEXTPARSER_WILDCARDS) / sizeof(wxString)); i++) {
    ...
    if (i + 1 < (sizeof(TEXTPARSER_WILDCARDS) / sizeof(wxString))) myReturnedWildcard.Append(_T("|"));
  }
...
bool TextParserTxtFile::CheckFileToParse() {
  ...
  if (myTestLine.Find(m_TextSeparator) == wxNOT_FOUND) {
    wxLogError(_("Format of %s seems to be incorrect. Parser is waiting for : %s"),
               m_ParseFileName.GetFullName().c_str(), GetParserType().c_str());
    return FALSE;
  }
  ...
  if (iFoundNbField != m_NbFieldToParse) {
    wxLogError(_("Number of fields not correct : found %d field(s), waiting for %d field(s)"), iFoundNbField,
               m_NbFieldToParse);
    return FALSE;
  }
...
TextParserTxtFileComma::TextParserTxtFileComma(const wxString &filename) : TextParserTxtFile(filename) {
  InitParserValue();
}
...
TextParserTxtFileTab::TextParserTxtFileTab(const wxString &filename) : TextParserTxtFile(filename) {
  InitParserValue();
}
