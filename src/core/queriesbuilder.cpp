...
bool QueriesBuilder::_CreateLayersQuery() {
  ...
  m_QueryData->m_QuerySQL =
      wxString::Format(myBaseQuery, myAtribTable.c_str(), TABLE_NAME_OBJECTS.c_str(), m_QueryData->m_QueryLayerID);
  return true;
}
  ...
  switch (m_QueryData->m_QueryFieldsStatus) {
    ...
    case AATTRIBUTION_EMPTY:
      myBaseQuery =
          ...
          _T(" ON (attrib.OBJECT_GEOM_ID = layer.OBJECT_ID) WHERE attrib.OBJECT_VAL_ID = %ld AND layer.OBJECT_ID IS ")
          _T("NULL");
    ...
    case AATTRIBUTION_YES:
      myBaseQuery = wxString::Format(
          ...
          myAtribTable.c_str(), TABLE_NAME_LAYER_AT.c_str(), m_QueryData->m_QueryLayerID, m_QueryData->m_QueryObjectID);
      ...
      for (unsigned int i = 0; i < m_QueryData->m_QueryFields.GetCount(); i++) {
        switch (m_QueryData->m_QueryFields.Item(i)->m_FieldType) {
          ...
          case TM_FIELD_ENUMERATION: {
            ...
            myBaseQuery.Append(
                wxString::Format(_T(" layer.%s = %ld AND"), m_QueryData->m_QueryFields.Item(i)->m_Fieldname, myValID));
          }
...
bool QueriesBuilder::_CreateObjectQuery() {
  wxString myBaseQuery =
      wxString::Format(_T("SELECT OBJECT_GEOM_ID from %s WHERE OBJECT_VAL_ID = %ld"),
                       TABLE_NAME_GIS_ATTRIBUTION[m_QueryData->m_QueryLayerType].c_str(), m_QueryData->m_QueryObjectID);
  m_QueryData->m_QuerySQL = myBaseQuery;
  return true;
}
