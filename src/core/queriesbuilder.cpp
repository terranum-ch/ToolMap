                ...
                switch (m_QueryData->m_QueryFields.Item(i)->m_FieldType) {
                    ...
                    case TM_FIELD_ENUMERATION: {
                        ...
                        for (unsigned int c = 0;
                             c < m_QueryData->m_QueryFields[i]->m_pCodedValueArray.GetCount(); c++) {
...
bool QueriesBuilder::_CreateDuplicateQuery()
{
    ...
    myQuery = wxString::Format(
            _T("SELECT a.OBJECT_ID FROM %s a, %s b WHERE ST_Equals(a.OBJECT_GEOMETRY, b.OBJECT_GEOMETRY) and a.OBJECT_ID<>b.OBJECT_ID"),
...
#if MYSQL_VERSION_ID >= 50600
    myQuery = wxString::Format(
            _T("SELECT a.OBJECT_ID FROM %s a, %s b WHERE ST_Crosses(a.OBJECT_GEOMETRY, b.OBJECT_GEOMETRY) and a.OBJECT_ID<>b.OBJECT_ID"),
            TABLE_NAME_GIS_GENERIC[m_QueryData->m_QueryLayerType].c_str(),
            TABLE_NAME_GIS_GENERIC[m_QueryData->m_QueryLayerType].c_str());
#else // 5.5 version didn't support strict spatial queries
