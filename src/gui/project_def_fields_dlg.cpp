...
void ProjectDefFieldDlg::OnImportAllowedValue(wxCommandEvent &event) {
  ...
  if (myImportSelector.ShowModal() == wxID_OK) {
    m_DlgAFD_Coded_Val_List->ImportParsedFileToListCtrl(myImportSelector.GetPath(), myImportSelector.GetFilterIndex());
  }
}
...
void ProjectDefFieldDlg::OnExportAllowedValue(wxCommandEvent &event) {
  ...
  if (myImportSelector.ShowModal() == wxID_OK) {
    m_DlgAFD_Coded_Val_List->ExportListParsedToFile(myImportSelector.GetPath(), myImportSelector.GetFilterIndex());
  }
}
...
void ProjectDefFieldDlg::OnShowLiveResults(wxCommandEvent &event) {
  ...
  if (iPrecision > 0 && iPrecision - iScale > 0) {
    ...
    for (i = 0; i < iLeft; i++) myTempResultString = myTempResultString + _T("0");
    myTempResultString.Append(_T("."));
    for (i = 0; i < iScale; i++) myTempResultString = myTempResultString + _T("0");
...
ProjectDefFieldDlg::ProjectDefFieldDlg(wxWindow *parent, PrjDefMemManage *myPrjMemManage, bool isEditingMode,
...
void ProjectDefFieldDlg::HideUnusedTabs(int iIndexShow) {
  for (int i = PRJDEF_FIELD_TYPE_NUMBER - 1; i >= 0; i--) {
    if (i != iIndexShow) m_DlgAFD_Choicebook->DeletePage(i);
  }
}
...
bool ProjectDefFieldDlg::TransferDataFromWindow() {
  ...
  wxString mySelectedTextValue = m_DlgAFD_Choicebook->GetPageText(m_DlgAFD_Choicebook->GetSelection());
...
void ProjectDefFieldDlg::CreateControls() {
  ...
  m_DlgAFD_Text_Length =
      new wxSpinCtrl(m_panel5, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 1000, 0);
  ...
  m_staticText11 = new wxStaticText(m_panel6, wxID_ANY,
                                    _("Integer fields may store numeric values :\n\nfrom -2147483647\nto 2147483647"),
  ...
  m_DlgAFD_Field_Precision = new wxSpinCtrl(m_panel7, ID_DLGAFD_FIELD_PRECISION, wxEmptyString, wxDefaultPosition,
  ...
  m_DlgAFD_Field_Scale = new wxSpinCtrl(m_panel7, ID_DLGAFD_FIELD_SCALE, wxEmptyString, wxDefaultPosition,
                                        wxDefaultSize, wxSP_ARROW_KEYS, 0, 100, 0);
  ...
  m_staticText111 = new wxStaticText(m_panel8, wxID_ANY, _("No options availlable for this type"), wxDefaultPosition,
