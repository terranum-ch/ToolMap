...
void ProjectDefFieldDlg::TransfertEnumValues(bool bSendDataToDlg) {
    ...
    if (bSendDataToDlg)  // DATA --> DIALOG
    {
        ...
        for (int i = 0; i < m_pPrjDefinition->GetCountCodedValue(); i++) {
            ProjectDefMemoryFieldsCodedVal* myCodedValObj = m_pPrjDefinition->GetActiveField()->m_pCodedValueArray.Item(
                i);
            // GetNextCodedValue();
...
void ProjectDefFieldDlg::CreateControls() {
    ...
    m_DlgAFD_Text_Length = new wxSpinCtrl(m_panel5, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
