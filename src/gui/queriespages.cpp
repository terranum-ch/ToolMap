...
QueriesPageIntro::QueriesPageIntro(QueriesWizard* parent, DataBaseTM* database)
    : wxWizardPage(parent) {
    wxASSERT(database);
    m_Parent = parent;
    m_pDB = database;
...
void QueriesPageIntro::CreateControls() {
    ...
    m_staticText1 = new wxStaticText(this, wxID_ANY, wxT("Choose which type of query you want to add"),
...
void QueriesPageLayer::_CreateControls() {
    ...
    m_staticText2 = new wxStaticText(this, wxID_ANY, _("Select all features from "), wxDefaultPosition, wxDefaultSize,
                                     0);
...
void QueriesPageObjectType::_CreateControls() {
    ...
    m_staticText8 = new wxStaticText(this, wxID_ANY, _("Choose the feature's type you want to select"),
                                     wxDefaultPosition, wxDefaultSize, 0);
...
void QueriesPageSelection::_CreateControls() {
    ...
    m_staticText7 = new wxStaticText(this, wxID_ANY, _("Select feature by object Kind"), wxDefaultPosition,
                                     wxDefaultSize, 0);
...
void QueriesPageAttribut1::_CreateControls() {
    ...
    wxStaticText* m_staticText7;
    m_staticText7 = new wxStaticText(this, wxID_ANY, _("Object attribute options"), wxDefaultPosition, wxDefaultSize,
                                     0);
    m_staticText7->Wrap(m_Parent->GetSize().GetWidth() - QUERIES_MARGIN_SIZE);
    bSizer10->Add(m_staticText7, 0, wxALL, 5);
...
void QueriesPageAttribut2::_CreateControls() {
    ...
    m_AdvText = new wxStaticText(this, wxID_ANY, _("If needed, delete unwanted criteria"), wxDefaultPosition,
                                 wxDefaultSize, 0);
...
QueriesPageAttribut2::QueriesPageAttribut2(QueriesWizard* parent, DataBaseTM* database, wxWizardPageSimple* prev,
    ...
    m_ReloadButton->Connect(EVT_FLATBUTTON_CLICKED, wxCommandEventHandler(QueriesPageAttribut2::OnReloadAttributs),
                            NULL, this);
}
...
QueriesPageGenericData::QueriesPageGenericData() {
    _Add(TOC_NAME_LINES, _("Line(s) with no attribution"),
         ...
         _T("generic_aat)"));
    _Add(
        TOC_NAME_LINES, _("Line(s) with more than one attribution"),
    ...
    _Add(
        TOC_NAME_LINES, _("Line(s) with one attribution"),
        _T("SELECT l.OBJECT_ID FROM generic_lines l WHERE OBJECT_ID IN (SELECT OBJECT_GEOM_ID  FROM generic_aat GROUP ")
        _T("BY OBJECT_GEOM_ID HAVING COUNT(*) = 1)"));
    _Add(
        TOC_NAME_POINTS, _("Point(s) with no attribution"),
    ...
    _Add(TOC_NAME_POINTS, _("Point(s) with more than one attribution"),
         _T("SELECT l.OBJECT_ID FROM generic_points l WHERE OBJECT_ID IN (SELECT OBJECT_GEOM_ID  FROM generic_pat ")
         _T("GROUP ")
         _T("BY OBJECT_GEOM_ID HAVING COUNT(*) > 1)"));
    _Add(TOC_NAME_POINTS, _("Point(s) with one attribution"),
         ...
         _T("BY OBJECT_GEOM_ID HAVING COUNT(*) = 1)"));
    _Add(
        TOC_NAME_LABELS, _("Label(s) with no attribution"),
    ...
    _Add(TOC_NAME_LABELS, _("Label(s) with more than one attribution"),
         _T("SELECT l.OBJECT_ID FROM generic_labels l WHERE OBJECT_ID IN (SELECT OBJECT_GEOM_ID  FROM generic_lat ")
         _T("GROUP ")
         _T("BY OBJECT_GEOM_ID HAVING COUNT(*) > 1)"));
...
bool QueriesPageExpert::TransferDataFromWindow() {
    ...
    if (m_Parent->GetData()->m_QueryType == QUERY_DUPLICATE) {
        m_Parent->GetData()->m_QueryName = wxString::Format(
            _("(duplicate) -  %s"), TOC_GENERIC_NAME_STRING[m_LayerType->GetSelection()].c_str());
    }

    if (m_Parent->GetData()->m_QueryType == QUERY_CROSSING) {
        m_Parent->GetData()->m_QueryName = wxString::Format(
            _("(crossing) -  %s"), TOC_GENERIC_NAME_STRING[m_LayerType->GetSelection()].c_str());
    }
    return true;
}
...
void QueriesPageGeomLine::_CreateControls() {
    ...
    m_staticText13 = new wxStaticText(this, wxID_ANY, _("Select lines shorter than (map units)"), wxDefaultPosition,
                                      wxDefaultSize, 0);
    m_staticText13->Wrap(m_Parent->GetSize().GetWidth() - QUERIES_MARGIN_SIZE);
    bSizer15->Add(m_staticText13, 0, wxALL, 5);

    m_SpinLineSize = new wxSpinCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 1,
                                    10000, 10);
...
void QueriesPageGeomNodes::_CreateControls() {
    ...
    m_staticText14 = new wxStaticText(this, wxID_ANY, _("Select lines with number of nodes less than"),
                                      wxDefaultPosition, wxDefaultSize, 0);
    m_staticText14->Wrap(-1);
    bSizer16->Add(m_staticText14, 0, wxALL, 5);

    m_SpinFewNodes = new wxSpinCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 2,
                                    10000, 2);
