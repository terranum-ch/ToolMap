...
void Shortcut_Panel_DLG::CreateControls() {
  ...
  m_ShortcutKey = new wxChoice(this, ID_DLG_SHORTCUT_LIST, wxDefaultPosition, wxDefaultSize,
                               (sizeof(tmShortCutKeys) / sizeof(wxString)), tmShortCutKeys);
  ...
  m_TypeList = new tmCheckListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize);
...
bool Shortcut_Panel_DLG::SetTypeList(DataBaseTM *database, int layer_type, int key) {
  ...
  while (1) {
    if (m_pDB->DataBaseGetNextResultAsObject(&myTempObject, layer_type)) {
      m_TypeList->AddItem(-1, myTempObject.m_ObjectID, myTempObject.m_ObjectName);
    } else
      break;
  }
    ...
    while (1) {
      if (m_pDB->GetNextShortCutObject(myShortcutID, key, bFirstLoop)) {
        ...
        for (unsigned int i = 0; i < myListCountItem; i++) {
          m_TypeList->GetItem(i, myCheckedID, mytemp, myChecked);
          if (myCheckedID == myShortcutID) m_TypeList->Check(i, true);
        }
