...
void ProjectPropertiesDLG::OnSaveButton(wxCommandEvent &event) {
  ...
  if (m_PrjDefinition.m_StoreDeleteScale.GetCount() > 0) m_DBHandler->DeleteMultipleScales(&m_PrjDefinition);
...
ProjectPropertiesDLG::ProjectPropertiesDLG(wxWindow *parent, DataBaseTM *database, wxWindowID id,
...
void ProjectPropertiesDLG::_CreateControls() {
  ...
  m_DLGPS_Export_Path =
      new wxDirPickerCtrlBest(itemPanel4, ID_DLGPS_EXPORT_PATH, wxEmptyString, _("Select an export path"));
  ...
  m_DLGPS_Backup_Path =
      new wxDirPickerCtrlBest(itemPanel4, ID_DLGPS_BACKUP_PATH, wxEmptyString, _("Select a backup folder"));
  ...
  m_DLGPS_Scale_List = new ScaleList(itemPanel16, ID_DLGPS_SCALE_LIST, m_DBHandler, &m_PrjDefinition);
  ...
  m_DLGPS_Scale_Order_Choice =
      new wxChoice(itemPanel16, ID_DLGPS_SCALE_ORDER_CHOICE, wxDefaultPosition, wxDefaultSize,
                   sizeof(PRJPROPERTIES_SCALE_ORDER_STRING) / sizeof(wxString), PRJPROPERTIES_SCALE_ORDER_STRING);
  m_DLGPS_Scale_Order_Choice->SetSelection(SCALE_ORDER_USER);
...
void ScaleList::BeforeAdding() {
  ...
  wxNumberEntryDialog *myScaleDialog =
      new wxNumberEntryDialog(this, _("Add a new scale"), _T("1:"), _("Adding new scale"), 1, 1, 999999999);
...
void ScaleList::BeforeEditing() {
  ...
  // create the dialog
  wxNumberEntryDialog *myDialog = new wxNumberEntryDialog(this, _("Edit the selected scale"), _T("1:"),
                                                          _("Editting scale"), m_ActualScale, 1, 999999999);
  SetDialog(myDialog);
}
...
void ScaleList::AfterEditing(bool bRealyEdited) {
  ...
  wxLogDebug(_T("Size of the scale array is : %d"), m_pPrjDefinition->m_ScaleArray.GetCount());
...
void ScaleList::BeforeDeleting() {
  ...
  wxLogDebug(_T("Nb of scale to delete (from DB) : %d"), m_pPrjDefinition->m_StoreDeleteScale.GetCount());
}
