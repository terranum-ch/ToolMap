...
void ProjectDefLayersDlg::OnChangeOrientation(wxCommandEvent &event) {
  ...
  if (m_DlgPDL_Orientation_FLD->IsChecked()) {
    ...
    for (int i = 0; i < iNbItem; i++) {
      if (i != iIndex) {
        if (m_DlgPDL_Fields_List->IsOrientationAllowed(i)) m_DlgPDL_Fields_List->SetOrientation(TM_FIELD_ORIENT_NO, i);
      } else
...
void ProjectDefLayersDlg::CreateControls() {
  ...
  if (!m_bIsModeEditing) {
    ...
    wxFlatButton *itemFlatButton15 =
        new wxFlatButton(m_DlgPDL_Panel_Obj, ID_DLGPDL_OBJECT_REMOVE, wxFLATBUTTON_TEXT_REMOVE);
    itemBoxSizer13->Add(itemFlatButton15, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxTOP | wxBOTTOM, 5);

    wxFlatButton *itemFlatButton16 =
        new wxFlatButton(m_DlgPDL_Panel_Obj, ID_DLGPDL_OBJECT_IMPORT, _("Import"), wxDefaultSize);
  ...
  m_DlgPDL_Fields_List =
      new ProjectDefLayersFieldsList(m_DlgPDL_Panel_Fields, ID_DLGPDL_FIELDS_LIST, wxSize(100, 100), this);
  ...
  wxFlatButton *itemFlatButton22 =
      new wxFlatButton(m_DlgPDL_Panel_Fields, ID_DLGPDL_FIELD_REMOVE, wxFLATBUTTON_TEXT_REMOVE);
  ...
  if (!m_bIsModeEditing) {
    ...
    m_DlgPDL_Contour_Name = new wxTextCtrl(itemDialog1, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, 0);
