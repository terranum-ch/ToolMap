...
void tmOpenError_DLG::_CreateControls(const wxString& projname, int iActDBVersion) {
    ...
    wxString myErrTxt2 = wxString::Format(
        _("This project was created with an older version of ToolMap. It must be updated before "
          "use.\nUpdated projects can no longer be loaded by previous versions of ToolMap.\n\nYour "
          "project version: %d, required project version: %d"),
        iActDBVersion, TM_DATABASE_VERSION);

    m_ErrVerTextCtrl = new wxStaticText(m_ErrPanelProjVersion, wxID_ANY, myErrTxt2, wxDefaultPosition, wxDefaultSize,
                                        0);
    ...
    m_ErrVerBackupConvertCtrl = new wxButton(m_ErrPanelProjVersion, wxID_ANY, _("Backup, convert and open..."),
    ...
    m_ErrVerConvertCtrl = new wxButton(m_ErrPanelProjVersion, wxID_ANY, _("Convert and open..."), wxDefaultPosition,
                                       wxDefaultSize, 0);
    ...
    wxString myErrTxt3 = _(
        "This project was created with a newer version of ToolMap. It is not supported by the installed version of "
    ...
    m_ErrTMDownloadCtrl = new wxButton(m_ErrPanelTMVersion, wxID_ANY, _("Download new version of ToolMap..."),
                                       wxDefaultPosition, wxDefaultSize, 0);
...
tmOpenError_DLG::tmOpenError_DLG(wxWindow* parent, int iActDBVersion, const wxString& projname, DataBaseTM* database,
    ...
    m_ErrVerBackupConvertCtrl->Connect(wxEVT_COMMAND_BUTTON_CLICKED,
                                       wxCommandEventHandler(tmOpenError_DLG::OnConvertBackup), NULL, this);
    m_ErrVerConvertCtrl->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(tmOpenError_DLG::OnConvert), NULL,
                                 this);
    m_ErrTMDownloadCtrl->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(tmOpenError_DLG::OnDownload), NULL,
                                 this);
}

tmOpenError_DLG::~tmOpenError_DLG() {
    m_ErrVerBackupConvertCtrl->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED,
                                          wxCommandEventHandler(tmOpenError_DLG::OnConvertBackup), NULL, this);
    m_ErrVerConvertCtrl->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(tmOpenError_DLG::OnConvert),
                                    NULL, this);
    m_ErrTMDownloadCtrl->Disconnect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(tmOpenError_DLG::OnDownload),
                                    NULL, this);
}
...
void tmOpenError_DLG::OnConvertBackup(wxCommandEvent& event) {
    ...
    myBckFile.SetInputDirectory(wxFileName(m_pDB->DataBaseGetPath(), m_pDB->DataBaseGetName()));
    myBckFile.SetDate(wxDateTime::Now());
    myBckFile.SetOutputName(wxFileName(myBkpPath, m_pDB->DataBaseGetName(), "tmbk"));
    myBckFile.SetComment(
...
void tmOpenRecentError_DLG::_CreateControls(const wxString& prjname) {
    ...
    wxString myMessage = wxString::Format(
        _("The project '%s' was not found!\nIt was maybe moved or deleted. \nPlease try opening another project"),
        prjname);
    ...
    m_RemoveCheckCtrl = new wxCheckBox(this, wxID_ANY, _("Remove this project from recent projects"), wxDefaultPosition,
                                       wxDefaultSize, 0, wxGenericValidator(&m_RemoveCheck));
...
tmOpenRecentError_DLG::tmOpenRecentError_DLG(wxWindow* parent, wxWindowID id, const wxString& title,
                                             const wxString& prjname, const wxPoint& pos, const wxSize& size,
                                             long style)
