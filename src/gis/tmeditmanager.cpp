...
/***************************************************************************/ /**
  ...
  *******************************************************************************/
tmEditManager::tmEditManager(ToolMapFrame *parent, tmTOCCtrl *toc, tmSelectedDataMemory *seldata, tmRenderer *renderer,
                             tmGISScale *scale) {
  InitMemberValues();
  m_ParentEvt = parent;
  m_TOC = toc;
...
void tmEditManager::OnToolOrientedPoint() {
  ...
  if (IsModifictionAllowed() == false) return;

  if (IsLayerTypeSelected(TOC_NAME_POINTS) == false) return;
...
bool tmEditManager::DeleteSelected(bool Clearselection) {
  ...
  for (unsigned int i = 0; i < mySelectedIds->GetCount(); i++) {
    wxArrayLong myFeatures = m_pDB->GetObjectsFromFeature(mySelectedIds->Item(i), m_TOC->GetEditLayer()->GetType());
...
bool tmEditManager::CreateIntersections() {
  ...
  for (unsigned int i = 0; i < myLinesCrossing->GetCount(); i++) {
    ...
    if (myCrossedL) {
      ...
      mySelLayer->SplitGeometry(&myRes2, myLinesCrossing->Item(i), myInsertedIDs2, m_TOC->GetEditLayer()->GetType());
      myRes1.empty();
      myRes2.empty();
...
bool tmEditManager::EditVertexPosition() {
  // validation
  if (IsModifictionAllowed() == false) return false;
