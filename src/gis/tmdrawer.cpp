...
bool tmDrawer::DrawLines(tmLayerProperties* itemProp, tmGISData* pdata) {
    ...
    while (1) {
        ...
        if (m_ActuallayerID == m_SelMem->GetSelectedLayer() && myProperty.IsEditing() && m_SelMem->GetCount() == 1 &&
...
#ifdef __WXMSW__
            const char* myValue = featureref->GetFieldAsString(
                myText.SubString(2, myText.Len() - 2).mb_str(wxConvUTF8));
