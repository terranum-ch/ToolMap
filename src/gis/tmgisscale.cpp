...
double tmGISScale::MetersToRealUnits(int meters) {
  ...
  double horizcoorddiff =
      std::max(m_ExtentWndReal.x_min, m_ExtentWndReal.x_max) - std::min(m_ExtentWndReal.x_max, m_ExtentWndReal.x_min);
...
void tmGISScale::_ComputeUnitScale() {
  ...
  // take mean latitude
  double myYmean =
      std::max(m_ExtentWndReal.y_max, m_ExtentWndReal.y_min) - std::min(m_ExtentWndReal.y_max, m_ExtentWndReal.y_min);
