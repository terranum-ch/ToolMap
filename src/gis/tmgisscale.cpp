...
tmRealRect tmGISScale::ComputeMaxCoord(const tmRealRect& r1, const tmRealRect& r2) {
    ...
    if (r1 == tmRealRect(0, 0, 0, 0)) return r2;
    if (r2 == tmRealRect(0, 0, 0, 0)) return r1;

    if (r1 == r2) return r1;

    tmRealRect retRect(r2.x_min, r2.y_min, r2.x_max, r2.y_max);

    if (r1.x_min < r2.x_min) retRect.x_min = r1.x_min;
    if (r1.y_min < r2.y_min) retRect.y_min = r1.y_min;
    if (r1.x_max > r2.x_max) retRect.x_max = r1.x_max;
    if (r1.y_max > r2.y_max) retRect.y_max = r1.y_max;
...
double tmGISScale::ComputeDivFactor(wxSize wnd_extent) {
    if (wnd_extent == wxDefaultSize) wnd_extent = wxSize(m_ExtentWnd.GetWidth(), m_ExtentWnd.GetHeight());

    double dx = 0, dy = 0;

    // check data
    if (wnd_extent == wxSize(0, 0) || m_ExtentMaxLayers == tmRealRect(0, 0, 0, 0)) return 0.0;
...
bool tmGISScale::ComputeMaxExtent() {
    ...
    wxSize myMarginSize(m_ExtentWnd.GetWidth() - tmSCALE_MARGIN, m_ExtentWnd.GetHeight() - tmSCALE_MARGIN);
...
void tmGISScale::ComputeNewRealZoomExtent(const wxRect& calc_wnd_extent, const wxPoint& top_left) {
    ...
    m_ExtentWndReal.y_min = RemoveFromCoord(m_ExtentWndReal.y_max,
                                            (((double)calc_wnd_extent.GetHeight()) * dOldPxSize));
...
double tmGISScale::MetersToRealUnits(int meters) {
    if (m_ProjectUnit == UNIT_METERS) return meters;
...
void tmGISScale::_ComputeUnitScale() {
    ...
    // take mean latitude
    double myYmean = std::max(m_ExtentWndReal.y_max, m_ExtentWndReal.y_min) -
                     std::min(m_ExtentWndReal.y_max, m_ExtentWndReal.y_min);
    ...
    // window size in m
    double my_wnd_size_m_x = (double)m_ExtentWnd.width * my_screen_size_mm.GetWidth() / my_screen_size_px.GetWidth() /
                             1000.0;
...
bool tmGISScale::IsLayerExtentValid() {
    if (m_ExtentMaxLayers == tmRealRect(0, 0, 0, 0)) return FALSE;
    return TRUE;
}
...
void tmGISScale::ComputeNewScaleExtent(const long& scale) {
    ...
    double my_wnd_size_m_x = (double)m_ExtentWnd.width * my_screen_size_mm.GetWidth() / my_screen_size_px.GetWidth() /
                             1000.0;
    double my_wnd_size_m_y = (double)m_ExtentWnd.height * my_screen_size_mm.GetHeight() /
                             my_screen_size_px.GetHeight() / 1000.0;
