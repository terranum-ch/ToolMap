...
bool tmTOCCtrl::RemoveLayer(wxTreeItemId position, bool bRemoveChild) {
  ...
  if (bRemoveChild) DeleteChildren(position);
  Delete(position);
...
unsigned int tmTOCCtrl::GetCountLayers() {
  if (m_root.IsOk()) return GetChildrenCount(m_root, FALSE);
  return 0;
}
...
bool tmTOCCtrl::MoveLayers(const wxTreeItemId &item, int newpos) {
  ...
  Freeze();
  wxTreeItemId newinserteditem = InsertItem(m_root, newpos, item1_text, item1_image, -1, newinserteditemData);
