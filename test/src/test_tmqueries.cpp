  ...
  protected:
    ...
    virtual void SetUp() {
        ...
        m_DataGeneric->m_QuerySQL =
            _T("SELECT l.OBJECT_ID FROM generic_lines l WHERE l.OBJECT_ID NOT IN (SELECT generic_aat.OBJECT_GEOM_ID ")
            _T("FROM ")
...
TEST_F(TestQueries, DeleteFields) {
    ...
    m_DataSelected->m_QueryObjectGeomID = 140;  // faille avec attrib avancés
    ASSERT_TRUE(
        m_DataSelected->GetFieldsValues(m_pDB, 7, m_DataSelected->m_QueryFields, m_DataSelected->m_QueryFieldsValues));
