...
bool siAttribut::LoadFromArray(const wxArrayString& attribtxt, DataBase* database, long layerindex) {
    ...
    // get attribut ID
    wxString myQuery = wxString::Format(
        _T("SELECT ATTRIBUT_ID FROM dmn_layer_attribut WHERE ATTRIBUT_NAME = '%s' AND LAYER_INDEX=%ld"),
...
bool siAttribut::Process(OGRFeature* feature, DataBase* database, long layerindex, long databaseid, long kind) {
    ...
    if (m_AttributOperation == SIATTRIBUT_OPERATION_VALUE) {
        ...
        wxString myQuery =
            _T("INSERT INTO layer_at%ld (OBJECT_ID, %s) VALUES (%ld,%ld) ON DUPLICATE KEY UPDATE %s=%ld");
    ...
    if (m_AttributOperation == SIATTRIBUT_OPERATION_REPLACE) {
        ...
        wxString myQuery =
            _T("INSERT INTO layer_at%ld (OBJECT_ID, %s) VALUES (%ld,%ld) ON DUPLICATE KEY UPDATE %s=%ld");
        if (!database->DataBaseQueryNoResults(wxString::Format(myQuery, layerindex, m_AttributNameOut, databaseid,
                                                               myOutCode, m_AttributNameOut, myOutCode))) {
            return false;
        }
        return true;
    }
    ...
    if (m_AttributOperation == SIATTRIBUT_OPERATION_COPY) {
        ...
        if (myFieldType == OFTString) {
            myQuery =
                _T("INSERT INTO layer_at%ld (OBJECT_ID, %s) VALUES (%ld,\"%s\") ON DUPLICATE KEY UPDATE %s=\"%s\"");
        } else {
        ...
        if (!database->DataBaseQueryNoResults(wxString::Format(myQuery, layerindex, m_AttributNameOut, databaseid,
            ...
            wxLogError(wxString::Format(myQuery, layerindex, m_AttributNameOut, databaseid, myFieldValue,
                                        m_AttributNameOut, myFieldValue));
            return false;
        }
        return true;
    }
